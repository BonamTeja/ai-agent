...
<p><strong>Strict Equal (===):</strong> It compares both values and types. It returns false if both types are different and does not perform type conversion.</p>
    ...
    value: [
      {
        command: "How does Node.js work?",
        text: `<p>Node.js works on a single-threaded, event-driven architecture using the V8 JavaScript engine.</p>
          
          <p><b>V8 Engine:</b> Compiles JavaScript into fast machine code.</p>
      ...
      {
        command: "What is NPM?",
        text: `<p>NPM stands for the Node Package Manager. It is the package manager for the NodeJS environment. It is used to install, share, and manage dependencies (libraries, tools, or packages) in JavaScript applications.</p>
          <p>NPM uses a package.json file in NodeJS projects to track project dependencies, versions, scripts, and metadata like the project's name and version.</p>
          <p>NPM is accessed by a command-line interface (CLI). Common commands include npm install to install packages, npm update to update them, and npm uninstall to remove them.</p>
       `,
      },
      {
        command: "Why is NodeJS single-threaded?",
        text: `<p>NodeJS is single-threaded because it's based on the asynchronous, non-blocking nature of JavaScript. This design makes it simpler to develop and maintain, and it allows NodeJS to handle many concurrent requests efficiently.</p>
       `,
      },
      {
        command: "If NodeJS is single-threaded, then how does it handle concurrency?",
        text: `<p>NodeJS is single-threaded, but it can handle concurrency efficiently through its event-driven, non-blocking I/O model.</p>
        <p>While the event loop in NodeJS runs on a single thread, it doesn’t block the execution of other tasks when waiting for I/O operations, such as file reads or database queries. Instead, NodeJS delegates these I/O tasks to the system's kernel, allowing it to continue processing other requests.</p>
        <p>Once the I/O operation is complete, the corresponding callback is added to a queue and processed by the event loop.</p>
       `,
      },
      {
        command: "Why is NodeJS preferred over other backend technologies like Java and PHP?",
        text: `
        <p><b>Fast Performance:</b> NodeJS is known for its speed in handling I/O-heavy tasks.</p>
        <p><b>NPM Ecosystem:</b> Node Package Manager offers over 50,000 bundles to help developers speed up development.</p>
        <p><b>Real-Time Applications:</b> Perfect for data-intensive, real-time apps as it doesn't wait for APIs to return data.</p>
      ...
      `,
          ...
          },
          {
            ...
            text: `<table border="1" cellSpacing="0" cellPadding="6">
              ...
              <tbody>
                <tr><td>Executes the callback immediately after the current event loop phase.</td><td>Executes the callback after the specified delay (in ms).</td></tr>
                <tr><td>Adds the callback to the next iteration of the event loop, in the check phase.</td><td>Adds the callback to the timer queue, to be executed after the specified delay.</td></tr>
